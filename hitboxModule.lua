...
function this.IsPointInArc(pointHitBox, arc)
    local angle = math.atan2(pointHitBox.y - arc.y, pointHitBox.x - arc.x) % (2 * math.pi)
    return  math.abs(arc.angle - angle) <= arc.amplitude and this.dist(pointHitBox, arc) <= arc.radius
end
